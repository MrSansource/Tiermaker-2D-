...
function rebuildContainersForAxes(
  ...
  function imageSearchQuery(name: string) {
    const suffix = imageSearchSuffix.trim();
    return suffix ? `${name} ${suffix}` : name;
  }

  async function findWikipediaImage(name: string) {
    const res = await fetch(`/api/wiki-image?q=${encodeURIComponent(imageSearchQuery(name))}`);
    if (!res.ok) return "";
    const data = await res.json();
  ...
  async function findPinterestImage(name: string) {
    const res = await fetch(`/api/serpapi-image?q=${encodeURIComponent(`${imageSearchQuery(name)} site:pinterest.com`)}`);
  ...
  async function findBrightDataImage(name: string) {
    const res = await fetch(`/api/brightdata-image?q=${encodeURIComponent(imageSearchQuery(name))}`);
    const data = await res.json().catch(() => ({}));
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="grid gap-1 sm:max-w-xl">
              <Label htmlFor="image-search-suffix">Suffixe de recherche image</Label>
              <Input
                id="image-search-suffix"
                className={INPUT_DARK}
                value={imageSearchSuffix}
                onChange={(e) => setImageSearchSuffix(e.target.value)}
                placeholder="Ex. photo officielle, album cover, musculation..."
              />
              <p className={cx("text-xs", T.mutedText)}>
                Ajoute apres chaque nom de tuile pour les recherches Wikipedia, Pinterest et Bright Data.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                variant="outline"
                className={OUTLINE_DARK}
                disabled={isImagePrefillLoading || !pairsText.trim()}
